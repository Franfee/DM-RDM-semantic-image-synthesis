...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5, label_dim=None,
        resample_filter=[1,1], resample_proj=False, adaptive_scale=True, spade=False,
        ...
        if spade:
            self.norm0 = SPADEGroupNorm(norm_nc=in_channels, label_nc=label_dim, eps=eps)
        else:
        ...
        if spade:
            self.norm1 = SPADEGroupNorm(norm_nc=out_channels, label_nc=label_dim, eps=eps)
        else:
    ...
    def forward(self, x, emb, cond=None):
        ...
        if self.spade:
            x = self.conv0(silu(self.norm0(x, cond)))
        else:
        ...
        if self.adaptive_scale:
            ...
            if self.spade:
                x = silu(torch.addcmul(shift, self.norm1(x, cond), scale + 1))
            else:
                x = silu(torch.addcmul(shift, self.norm1(x), scale + 1))
        else:
            if self.spade:
                x = silu(self.norm1(x.add_(params), cond))
            else:
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        label_dim           = None,         # Number of class labels
        ...
        block_kwargs = dict(emb_channels=emb_channels, channels_per_head=64, dropout=dropout, init=init, init_zero=init_zero, eff_attn=eff_attn, label_dim=label_dim)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True, spade=True, **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, spade=True, **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, spade=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=(res in attn_resolutions), spade=True, **block_kwargs)
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        x:              [B,C,H,W]
        noise_labels:   [B]
        class_labels:   [B,L,H,W]
        augment_labels: [B, dim=9]
        """
        # Mapping.
        emb = self.map_noise(noise_labels)                               # [B] ->        [B, model_channels=192/256]
        if self.map_augment is not None and augment_labels is not None:
            emb = emb + self.map_augment(augment_labels)                 # [B, dim=9] -> [B, model_channels=192/256]
        emb = silu(self.map_layer0(emb))                                 # [B, model_channels=192/256] -> [B, dim=768/1024]
        emb = self.map_layer1(emb)                                       # [B, dim=768/1024] -->           [B, dim=768/1024]
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels.to(dtype), **model_kwargs)
