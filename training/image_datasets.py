    ...
    *,
    ...
    if dataset_mode == 'cityscapes':
        ...
        for item in all_files:
            finame = str(item).split("/")[-1].split(".")[0]
            listA.append(finame)
        
        for item in classes:
            finame = str(item).split("/")[-1].split(".")[0]
            listB.append(finame)
        comm = list(set(listA).intersection(set(listB)))
        #comm.sort()
        all_files =[]
        classes = []

        for item in comm:
            all_files.append(os.path.join(data_dir, 'train' if is_train else 'test', 'images', str(item)+".jpg"))
            classes.append(os.path.join(data_dir, 'train' if is_train else 'test', 'labels', str(item)+".png"))
